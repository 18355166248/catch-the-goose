...
- 新模型出参考稿前，必须同时查看同主题已完成模型的主色、明度和饱和度；不能把“主题统一”
  简化成所有物件都使用同一种深绿、暗金或低明度材质。
- 相邻模型的主色或明度必须形成明显区分。连续两件已经使用深色后，下一件默认选择中高明度；
  同一主题至少同时具备冷/暖、明/暗和高/低饱和度层次。
- 主题统一优先依靠造型语言、金属做旧、纹样尺度、粗糙度和统一灯光，不依靠全组染成同色。
- 固定机位对照图中，目标模型必须能仅凭色块从相邻物件中分离；缩小后若主色发黑、发灰或与
  已完成模型混成一片，即使单件特写美观也不能通过。
- 古玩铺后续预设色彩分工：`banzhi` 使用浅青蓝玉/蜜糖皮色/暖金，`yuxi` 使用暖白玉/朱砂红/
  古金，`ruyi` 使用淡紫玉/孔雀蓝/暖金；建模时可微调，但不得全部退回深绿色。
