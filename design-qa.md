...
- [P1, fixed] Jolt 迁移后只在摘件时唤醒局部接触岛；平铺容器里休眠件没有足够势能，
  视觉上等于“消除后完全不碰撞”。新增 `JoltWorld.kickAround`：只对摘除点最近的 5 件
  （三消成立时 7 件）施加向外、向上及角向真实冲量，让反馈继续经过接触求解，不用与
  碰撞体脱节的 Transform 假动画，也不唤醒整堆。
- [P1, fixed] 投放动画原为 0.15 秒一次性 `backOut`，手机上仅约 9 帧且与自由落体重叠，
  体感近似闪现。现在逐件间隔为 0.06 秒，视觉子树执行 0.28 秒“110% 过冲 → 96% 回弹
  → 100% 落稳”三段动画；根节点与最终尺寸碰撞体仍由 Jolt 控制。
- 390×844 浏览器关键帧验证：开局 0.18 / 0.60 / 1.45 秒物件数量持续增长；摘走单件后
  黄环、玉牌、鹅和宝石在 80–700ms 内局部碰开并回落，远处件保持稳定；三只鹅连消后
  槽位清空、得分 +100、进度 8%，附近玉牌与圆环产生更明显的加强碰撞。
- Cocos Web 构建通过，最终浏览器控制台 0 warning / 0 error。

## 2026-08-11 古玩铺九种模型补齐验收

- 新增原创 Blender 程序化模型 `yuxi`（玉玺，396 面）与 `ruyi`（如意，1376 面）及
  对应透明槽位图标；古玩铺 family 从 7 种补齐到 9 种。
- 390×844 大师关现与其余主题一致为 54 个可消件 + 2 块石头；玉玺方形金边、如意细长
  云头与既有玉镯/手串/葫芦轮廓可区分。
- 新模型加载、冰封标记与点击提示正常；最终浏览器控制台 0 warning / 0 error。

## 2026-08-11 甜品小镇第四主题验收

### 交付与结果

- 第四站「甜品小镇」由锁定预告改为可玩；从池塘农场继续向下拖动即可看到，选中后扩展
  路线会自动把第四站居中。
- Blender 程序化生成 9 个甜品、陶瓷托盘及 9 张槽位图标；单件 60–1440 面。
- imagegen 原创生成 9:16 甜品街景，切为 720×1280 JPEG；中心空广场与独立托盘分层。
- 390×844 大师关成功加载完整 9 种甜品 + 2 块石头，共 56 件；物件全部位于托盘内。
- 拾取马卡龙后图标正确飞入第一格，验证了资源路径与槽位 Sprite 链路。
- 同轮修复槽位图标的响应式坐标 bug：Widget 位于固定 720×1280 `gameLayer`，图标现在
  使用同一父层高度定位，不再在短屏上落到“移出”按钮。
- 最终 Cocos Web 构建自检通过，浏览器控制台 0 warning / 0 error。

## 2026-08-11 池塘农场第三主题验收

### 交付

- 新增第三张可玩地图「池塘农场」，地图节点由锁定预告改为可选并支持自动滚动居中。
- Blender 程序化生成 9 个物件、藤编收获篮和 9 张 192×192 透明槽位图标；全部模型
  最大边归一化，单件 48–1738 面、12–84KB，容器 1998 面 / 112KB。
- imagegen 原创生成 9:16 场景源图，切为 720×1280 JPEG；中央留空承接独立 3D 篮筐。

### 浏览器证据（390×844）

- 地图向下拖动后能看到农场站点；点击后黄色鹅标记、站名和文案同步切换，并自动把扩展站
  拉回视口中央。
- 标准关成功加载背景、篮筐及前 6 种模型；拾取青蛙后对应图标正常飞入槽位。
- 大师关成功加载完整 9 种模型 + 2 块石头，共 56 件，所有物件均在篮筐内且可辨识。
- 验收发现并修复难度切换后的 HUD 陈旧状态：大师关现显示「第 3 关」，从 2:45 正常倒计时，
  不再沿用上一局的「第 2 关 / 3:30」。
- Cocos Web 构建通过 `jolt-glue.js` 与非空脚本包自检；最终浏览器控制台 0 warning / 0 error。
...
# 2026-08-12 发布可信化回归验收

## Evidence

- 正式构建：`game/build/web-mobile/`，release / WASM / MD5 关闭 / 无 source map 门禁通过。
- 浏览器视口：347×603 CSS px。
- 状态：首页、标准关对局、暂停菜单、退出回首页、二次开局、每日次数用尽与续次。

## Findings

- 首页四站切图、站点圆章与选中绿勾均正常渲染，此前圆章验收 blocker 已解除。
- 暂停面板四个操作在短屏内完整显示；“退出本局”返回地图后再次开局，无遗留节点或
  Jolt 幽灵刚体，此前退出链路 blocker 已解除。
- 今日次数从 3→2→1→0 正常扣减；0 次开局出现“再续一次”，确认后正常进入新局。
- 两条远程切图地址故障注入后，本地 fallback 保持首页构图完整；正式地址恢复后无新增
  warning/error。
- Jolt 关卡落堆、冰封标记、道具栏与收集槽在二次开局中正常。
